...
활용 상황: 한 예시로 회사에서 상품 및 서비스의 생산과 개발, 고객 관리 등을 위해 정보를 정리하고 저장할 때 활용할 수 있다. 
```
...
```
- SQL이 필요하다고 느낀 이유: 무수히 많은 데이터가 쌓여 있는 상태에서 이를 직접 정리하고 분석하는 것은 불가능하다. SQL을 통해서는 빠르고 정확하게 원하는 정보만을 추출해낼 수 있다. 
- SQL로 해보고 싶은 일: 그동안 다른 방식으로 분석했던 기존의 데이터셋들을 SQL로도 분석해보면서, 방식을 비교해보고 싶다. 
- 앞으로 SQL을 공부할 때 기대되는 점: 데이터의 개수도 많고 정제되지 않는 상태의 데이터셋을 마주하더라도 당황하지 않고 분석을 잘 해내고 싶다.  
```
...
```
1. SQL을 처음 써보면서 가장 낯설었던 점: 관련 개념을 처음 배워 전체적으로 낯설었던 것 같다. 
2. 다음 주에 더 익숙해지고 싶은 부분: 데이터 활용 과정 중 원하는 것을 정하는 일과, SQL을 통해 다량의 자료가 있을 때 이를 연결하는 부분에 대해 더 많은 학습과 경험을 쌓고 싶다. 
3. 과제 진행 중 막혔던 부분이 있다면: 아직 SQL을 본격적으로 활용한 것이 아니기 때문에 실제로 내가 어떻게 사용할 수 있을지에 대한 감이 잘 오지 않은 것 같다. 
```
